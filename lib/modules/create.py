...
class CreateVmCommandBundle(BaseCommands):
    ...
    @args('--tem', '--template', help='template object to use as a source of cloning')
    @args('--flavor', help='flavor to use for a vm cloning')
    @args('--dc', '--datacenter', help='datacenter where to create vm')
    @args('--folder', help='folder where to place vm')
    @args('--ds', '--datastore', help='datastore where to store vm')
    @args('--cl', '--cluster', help='cluster where to spawn mv')
    @args('--rpool', '--resource-pool', help='resource pool, which should be used for vm')
    @args('--mem', help='memory to set for a vm in megabytes')
    @args('--cpu', help='cpu count to set for a vm', type=int)
    ...
    @args('--gu', '--guest-user', help="guest's user under which to run command through vmtools")
    @args('--gp', '--guest-pass', help="guest user's password")
    def execute(self, args):
        flavor = load_vm_flavor(args.flavor)
